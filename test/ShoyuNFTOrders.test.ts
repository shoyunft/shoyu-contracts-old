...
describe("Test buy and sell orders", function () {
  before(async function () {
    ...
    this.ZeroExInitialMigration = await ethers.getContractFactory(
      "InitialMigration"
    );
    this.ZeroEx = await ethers.getContractFactory("ZeroEx");
    this.ZeroExERC721Orders = await ethers.getContractFactory(
      "ERC721OrdersFeature"
    );
    this.ZeroExRegistry = await ethers.getContractFactory(
      "SimpleFunctionRegistryFeature"
    );
    this.ZeroExOwnable = await ethers.getContractFactory("OwnableFeature");
    this.ZeroExFeeCollectorController = await ethers.getContractFactory(
      "FeeCollectorController"
    );
    this.ZeroExTransformERC20 = await ethers.getContractFactory(
      "TransformERC20Feature"
    );
    this.ZeroExMetaTransactions = await ethers.getContractFactory(
      "MetaTransactionsFeature"
    );
    this.ZeroExNativeOrders = await ethers.getContractFactory(
      "NativeOrdersFeature"
    );
    ...
    this.ZeroExUniswapFeature = await ethers.getContractFactory(
      "UniswapFeature"
    );

    /* shoyu contract factories */
    this.ShoyuERC721Orders = await ethers.getContractFactory(
  ...
  beforeEach(async function () {
    ...
    /* sushi deployments */
    this.sushiswapFactory = await this.UniswapV2Factory.deploy(
      this.minter.address
    );
    await this.sushiswapFactory.deployed();

    this.sushiswapRouter = await this.UniswapV2Router.deploy(
      this.sushiswapFactory.address,
      this.weth.address
    );
    await this.sushiswapRouter.deployed();

    /* seed sushi pool */
    const pair = await this.sushiswapFactory.createPair(
      this.weth.address,
      this.sushi.address
    );

    this.lp = await this.UniswapV2Pair.attach(
      (
        await pair.wait()
      ).events[0].args.pair
    );
    ...
    await this.weth
      .connect(this.minter)
      .approve(this.sushiswapRouter.address, MaxUint256);
    await this.sushi
      .connect(this.minter)
      .approve(this.sushiswapRouter.address, MaxUint256);
    ...
    /* 0x deployments */
    const fullMigration = await this.ZeroExFullMigration.deploy(
      this.minter.address
    );
    await fullMigration.deployed();

    this.zeroEx = await this.ZeroEx.deploy(
      await fullMigration.getBootstrapper()
    );
    ...
    const feeCollectorController =
      await this.ZeroExFeeCollectorController.deploy(
        this.weth.address,
        AddressZero
      );
    ...
    const metaTransactions = await this.ZeroExMetaTransactions.deploy(
      this.zeroEx.address
    );
    ...
    const otcOrders = await this.ZeroExOtcOrders.deploy(
      this.zeroEx.address,
      this.weth.address
    );
    ...
    const erc721Orders = await this.ZeroExERC721Orders.deploy(
      this.zeroEx.address,
      this.weth.address
    );
    ...
    const shoyuERC721Orders = await this.ShoyuERC721Orders.deploy(
      this.zeroEx.address,
      this.weth.address,
